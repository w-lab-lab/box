...
Page({
  ...
  async onLoad(options) {
    ...
    myUserId = myInfoAndMyUniversityInfo.openid
    user_uid = myInfoAndMyUniversityInfo.uid
    console.log("打印myUserId")
    ...
    res = await api.getCollectSituation(params)
    if (res.errno == -1) {
      console.log("收藏状态获取失败！")
      Dialog.alert({
        title: '出错了！',
        message: res.message,
      }).then(() => {
      ...
      CollectSituation = res.data.data[0]
      this.setData({
        collect_or_not: CollectSituation.collect
      })
      collect_or_not = CollectSituation.collect
      console.log({
        "收藏状态的值": CollectSituation
      })
    }
  ...
  onEnterChatroom(){
    if (this.data.myUserId == this.data.user_id) {
      Dialog.alert({
        title: '出错了！',
        message: "该商品为自己发布的商品，无法聊天",
      })
      return
    }
    wx.navigateTo({
      url: '../chatroom/chatroom?id=' + commodity_id,
    })
  },
  ...
  async collectthis() {
    ...
    if (this.data.myUserId == this.data.user_id) {
      Dialog.alert({
        title: '出错了！',
        message: "你不能收藏自己发布的商品！",
      })
      return
    }
    collect_or_not = !collect_or_not
    const collector = myUserId
    if (collect_or_not) {
      params = {
        commodity_id,
        collector
      }
      console.log("要传入api的参数", params)
      res = await api.setCollectSituation(params)
      if (res.errno != 0) {
      ...
      const id = CollectSituation._id
      params = {
        id
      }
      console.log("要传入api的参数值", id)
      res = await api.delCollectSituation(params)
      if (res.errno != 0) {
