...
Page({
  ...
  async onAuthentication() {
    res = await api.getUniversityInfoByUid(uid)
    if (res.data[0].location) {
      location[0] = res.data[0].location[0]
      location[1] = res.data[0].location[1]
      location[2] = res.data[0].location[2]
      location[3] = res.data[0].location[3]
    } else {
      wx.showToast({
        title: '该学校无需认证',
        icon: 'none',
    ...
    if (res.data[0].location) {
      ...
      wx.getLocation({
        success(res) {
          ...
          if (res.longitude > location[0] && res.longitude < location[1] && res.latitude > location[2] && res.latitude < location[3]) {
            wx.hideLoading()
            wx.showToast({
              title: '认证学校成功',
              icon: 'none',
