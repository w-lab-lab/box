...
const collectcommodity = db.collection('commodity_collection')
...
exports.main = async (event, context) => {
  ...
  app.router('setCommodityDetail', async (ctx, next) => {
    ...
    // 更新商品详细信息
    app.router('uploadCommodityDetail', async (ctx, next) => {
      try{
        ctx.body = await commodityCollection.where({
      ...
      .update({
        data:{
          ...
          img_url:event.params.img_url,
          number:event.params.number,
          origin_url:event.params.origin_url,
          price_now:event.params.price_now,
          price_origin:event.params.price_origin,
          remark:event.params.remark,
          thumbnail_url:event.params.thumbnail_url,
     ...
     //获取收藏商品_id
  app.router('getCollectionInfo', async (ctx, next) => {
    try {
      ctx.body = await collectcommodity.where({
          collector: wxContext.OPENID
        })
        .field({
          _id: false,
          commodity_id: true,
  ...
  //获取收藏列表
  app.router('getCollectionListBy_id', async (ctx, next) => {
    const {
      Collection,
      start,
      count,
  ...
  //通过用户openid和商品_id获取商品收藏状态
  app.router('getCollectSituation', async (ctx, next) => {
    const {
      comId,
      colId
    } = event.params
  ...
  //上传收藏状态
  app.router('setCollectSituation', async (ctx, next) => {
    try {
      ctx.body = await collectcommodity.add({
        data: {
          ...event.params,
          collect: true,
          create_time: db.serverDate(),
  ...
  //删除收藏状态
  app.router('delCollectSituation', async (ctx, next) => {
    const {
      id
    } = event.params
    try {
